                                    ...
                                    <div class="card">
                                        <div class="card-body">
                                            <h5 class="card-title budidaya-tahapan">
                                                <a href="{{ route('budidaya.show', $budidaya['id_budidayas']) }}">
                                                    {{ $budidaya['tahapan'] }}
                                                </a>
                                            </h5>
                                            <!-- Pindahkan tombol edit dan delete ke ujung kanan atas -->
                                            <div class="float-right">
                                                <a href="{{ route('budidaya.edit', $budidaya['id_budidayas']) }}"
                                                    class="btn btn-success btn-sm text-center"><i
                                                        class="fas fa-edit"></i></a>
                                                <form id="delete-form-{{ $budidaya['id_budidayas'] }}"
                                                    action="{{ route('budidaya.destroy', $budidaya['id_budidayas']) }}" method="POST"
                                                    class="d-inline delete-about-form">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button"
                                                        onclick="confirmDelete({{ $budidaya['id_budidayas'] }}, event)"
